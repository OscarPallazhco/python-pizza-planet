...
def test_create__without_beverages(app, ingredients, size, client_data):
    created_size, created_ingredients = __create_sizes_and_ingredients(ingredients, [size])
...
def test_create__should_return_message_when_required_info_is_missing(app, ingredients, beverages, size, client_data):
    created_size, created_ingredients = __create_sizes_and_ingredients(ingredients, [size])
    created_beverages = __create_beverages(beverages)
    order = __order(created_ingredients, created_beverages, created_size, client_data)
    del order["size_id"]
...
def test_create__should_return_invalid_size_message_when_size_id_not_exists(app, ingredients, beverages, size, client_data):
    created_size, created_ingredients = __create_sizes_and_ingredients(ingredients, [size])
    created_beverages = __create_beverages(beverages)
    order = __order(created_ingredients, created_beverages, created_size, client_data)
...
def test_calculate_order_price(app, ingredients, beverages, size, client_data):
    ...
    ingredients_total = sum(ingredient['price'] for ingredient in created_ingredients)
    beverages_total = sum(beverage['price'] for beverage in created_beverages)
    pytest.assume(created_order['total_price'] == round(created_size['price'] + ingredients_total + beverages_total, 2))


def test_calculate_order_price__without_beverages(app, ingredients, size, client_data):
    created_size, created_ingredients = __create_sizes_and_ingredients(ingredients, [size])
    order = __order(created_ingredients, [], created_size, client_data)
    created_order, _ = OrderController.create(order)
    ingredients_total = sum(ingredient['price'] for ingredient in created_ingredients)
    pytest.assume(created_order['total_price'] == round(created_size['price'] + ingredients_total, 2))
